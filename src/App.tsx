...
import { useState, useEffect, useMemo, Fragment, memo, useRef, Suspense, lazy } from 'react';
...
interface TaskTracking {
  id: string;
  activity: string;
  status: '⚠️ មិនទាន់ចាប់ផ្តើម' | '✅ បញ្ចប់រួចរាល់' | '🏃 កំពុងដំណើរការ' | '❌ ផ្អាក';
  startDate: string;
  deadline: string;
  responsiblePerson: string;
  documentType?: 'meeting' | 'feedback';
  typeColor?: string;
  fileUrl?: string;
  notes: string;
  order?: number;
...
// Memoized row component for Committee Modal to improve typing performance
const CommitteeRowItem = memo(({ row, index, onUpdate, onRemove, onFileUpload, onViewFile, isUploading }: any) => {
  return (
    <div key={row.id} className="p-4 border border-gray-100 dark:border-gray-800 rounded-xl hover:border-emerald-100 dark:hover:border-emerald-900/30 transition-all relative group">
      <div className="grid grid-cols-12 gap-3 items-start">
        <div className="col-span-12 md:col-span-3">
          ...
          <textarea 
            value={row.objective}
            onChange={e => onUpdate(row.id, { objective: e.target.value })}
            rows={1}
        ...
        <div className="col-span-6 md:col-span-2">
          ...
          <input 
            type="text" 
            value={row.ministry}
            onChange={e => onUpdate(row.id, { ministry: e.target.value })}
            className="w-full px-3 py-2 border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-xs transition-all"
            placeholder="ក្រសួង..."
          />
        </div>
        <div className="col-span-6 md:col-span-2">
          ...
          <input 
            ...
            onChange={e => onUpdate(row.id, { documentNumber: e.target.value })}
            className="w-full px-3 py-2 border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-xs transition-all"
            ...
            <label className="flex-1 flex items-center gap-1.5 px-2 py-2 border border-dashed border-gray-200 dark:border-gray-700 rounded-lg hover:border-emerald-500 transition-all cursor-pointer overflow-hidden">
              ...
              <input 
                ...
                onChange={e => {
                  ...
                  onFileUpload(row.id, file);
                }}
              />
            </label>
            {row.fileUrl && !row.file && (
              <button 
                onClick={() => onViewFile(row.fileUrl!)}
        ...
        <div className="col-span-1 flex items-end justify-center pb-1.5">
          <button 
            ...
            className="w-7 h-7 bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 rounded-lg flex items-center justify-center hover:bg-red-100 dark:hover:bg-red-900/40 transition-all disabled:opacity-30"
            title="លុបជួរដេក"
          >
...
// Memoized main components to prevent unnecessary re-renders of the whole app tree
const Dashboard = memo(({ officials, missions, currentDate, theme, isDarkMode }: { officials: Official[], missions: Mission[], currentDate: Date, theme: string, isDarkMode: boolean }) => {
  ...
  const totalLeaves = currentMonthMissions.reduce((acc, m) => acc + (m.leaves?.length || 0), 0);
  const uniqueOfficialsWithActivity = new Set(
    currentMonthMissions
      .filter(m => m.days.length > 0 || (m.leaves && m.leaves.length > 0))
      .map(m => m.officialId)
  ).size;
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="w-14 h-14 bg-red-50 dark:bg-red-900/10 rounded-2xl flex items-center justify-center">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{totalMissions} ថ្ងៃ</h3>
          </div>
        </motion.div>
        ...
        <motion.div 
          ...
          className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-4"
        >
          <div className="w-14 h-14 bg-yellow-50 dark:bg-yellow-900/10 rounded-2xl flex items-center justify-center">
            <AlertCircle className="w-8 h-8 text-yellow-600 dark:text-yellow-400" />
          </div>
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">សរុបថ្ងៃច្បាប់</p>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{totalLeaves} ថ្ងៃ</h3>
          </div>
        </motion.div>
        ...
        >
          ...
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">មន្ត្រីមានសកម្មភាព</p>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{uniqueOfficialsWithActivity} នាក់</h3>
          </div>
        </motion.div>
        ...
        >
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{uniqueWomenOnMission} នាក់</h3>
          </div>
        </motion.div>
      </div>
...
const CommitteesTable = memo(({ 
  ...
  const groupedCommittees = useMemo(() => {
    ...
    // Sort groups by position rank, then by name
    return Object.values(groups).sort((a, b) => {
      const leaderA = a[0];
      const leaderB = b[0];
      const rankA = positionRank[leaderA.leaderPosition] || 99;
      const rankB = positionRank[leaderB.leaderPosition] || 99;
      
      if (rankA !== rankB) return rankA - rankB;
      return leaderA.leaderName.localeCompare(leaderB.leaderName, 'km');
    });
  }, [committees]);
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
      <div>
        <table className="w-full border-collapse text-sm border-hidden">
          <thead>
            <tr className="bg-gray-50/80 dark:bg-gray-800/80">
              <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-12 border border-gray-200 dark:border-gray-700 uppercase tracking-wider text-[10px]">ល.រ</th>
              <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 w-48 border border-gray-200 dark:border-gray-700 uppercase tracking-wider text-[10px]">ឈ្មោះថ្នាក់ដឹកនាំ</th>
              <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 uppercase tracking-wider text-[10px]">កម្មវត្ថុ</th>
              <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 w-48 border border-gray-200 dark:border-gray-700 uppercase tracking-wider text-[10px]">ក្រសួង</th>
              <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 w-48 border border-gray-200 dark:border-gray-700 uppercase tracking-wider text-[10px]">លេខលិខិត</th>
              <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-24 border border-gray-200 dark:border-gray-700 uppercase tracking-wider text-[10px]">ឯកសារ</th>
              <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 w-32 border border-gray-200 dark:border-gray-700 uppercase tracking-wider text-[10px]">ផ្សេងៗ</th>
              <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-16 border border-gray-200 dark:border-gray-700 uppercase tracking-wider text-[10px]">ចែករំលែក</th>
            </tr>
          </thead>
          <tbody>
            {groupedCommittees.length === 0 ? (
              <tr>
                <td colSpan={8} className="p-8 text-center text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700">
              ...
              groupedCommittees.map((group, gIdx) => (
                <tr 
                  key={group[0].id} 
                  className="hover:bg-gray-50/10 dark:hover:bg-gray-800/10 transition-colors group"
                >
                  <td className="p-4 text-center text-gray-500 dark:text-gray-400 align-top border border-gray-200 dark:border-gray-700 font-mono font-bold text-xs">
                    {gIdx + 1}
                  </td>
                  <td className="p-4 align-top border border-gray-200 dark:border-gray-700">
                    <div className="font-bold text-gray-900 dark:text-gray-100">{group[0].leaderName}</div>
                    <div className="text-[10px] text-gray-500 dark:text-gray-400 font-medium">{group[0].leaderPosition}</div>
                    {isEditor && (
                      <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-all mt-2">
                        <button 
                          onClick={() => onEdit(group)} 
                          className="p-1 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg flex items-center gap-1 text-[10px] font-bold border border-blue-100 dark:border-blue-800 w-full justify-center transition-all"
                        >
                          <Settings className="w-3 h-3" /> កែសម្រួល
                        </button>
                        <button 
                          onClick={() => onDelete(group.map(item => item.id))} 
                          className="p-1 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg flex items-center gap-1 text-[10px] font-bold border border-red-100 dark:border-red-800 w-full justify-center transition-all"
                        >
                  ...
                  <td colSpan={5} className="p-0 align-top border border-gray-200 dark:border-gray-700">
                    <div className={cn(
                      ...
                      <table className="w-full border-collapse border-hidden">
                        <tbody>
                          {group.map((c, iIdx) => (
                            ...
                            >
                              <td className="p-4 align-top border-r border-gray-100 dark:border-gray-800">
                                <div className="text-gray-700 dark:text-gray-300 flex items-start gap-2">
                                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" />
                                  <span className="whitespace-pre-wrap leading-relaxed">{c.objective}</span>
                                </div>
                              </td>
                              <td className="p-4 align-top w-48 border-r border-gray-100 dark:border-gray-800">
                                <div className="text-gray-700 dark:text-gray-300 flex items-start gap-2">
                                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-blue-400 shrink-0" />
                                  <span className="font-medium text-xs">{c.ministry || '-'}</span>
                                </div>
                              </td>
                              <td className="p-4 align-top w-48 border-r border-gray-100 dark:border-gray-800">
                                <div className="text-gray-700 dark:text-gray-300 flex items-start gap-2">
                                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-purple-400 shrink-0" />
                                  <span className="font-bold text-xs">{c.documentNumber || '-'}</span>
                                </div>
                              </td>
                              <td className="p-4 align-top text-center w-24 border-r border-gray-100 dark:border-gray-800">
                                {c.fileUrl ? (
                                  <button 
                                    onClick={() => onViewFile(c.fileUrl!)}
                                    className="p-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all inline-block cursor-pointer shadow-sm"
                                    title="មើលឯកសារ"
                                  >
                              ...
                              <td className="p-4 align-top w-32">
                                <div className="text-gray-500 dark:text-gray-400 text-[10px] italic leading-relaxed">
                                  {c.notes || '-'}
                                </div>
                              </td>
                            </tr>
                  ...
                  <td className="p-4 align-top text-center border border-gray-200 dark:border-gray-700">
                    <button 
...
export default function App() {
  ...
  const [logs, setLogs] = useState<UserHistoryLog[]>([]);
  ...
  const [bulkUpdateType, setBulkUpdateType] = useState<'mission' | 'leave' | 'none'>('none');
  ...
  const [mainView, setMainView] = useState<'missions' | 'committees' | 'tracking'>('missions');
  const [tasks, setTasks] = useState<TaskTracking[]>([]);
  const [taskSearchQuery, setTaskSearchQuery] = useState('');
  const [isAddingTask, setIsAddingTask] = useState(false);
  const [editingTask, setEditingTask] = useState<TaskTracking | null>(null);
  const [newTask, setNewTask] = useState<Partial<TaskTracking>>({
    activity: '',
    status: '⚠️ មិនទាន់ចាប់ផ្តើម',
  ...
  const [taskTrackingDate, setTaskTrackingDate] = useState(new Date());
  
  const filteredTasks = useMemo(() => {
    let list = [...tasks];

    // Filter by selected month and year
    const selectedMonth = getMonth(taskTrackingDate);
    const selectedYear = getYear(taskTrackingDate);
    
    list = list.filter(task => {
      const taskDate = task.deadline ? new Date(task.deadline) : (task.createdAt ? new Date(task.createdAt) : null);
    ...
    if (taskSearchQuery.trim()) {
      ...
      list = list.filter(t => 
        ...
        (t.responsiblePerson && t.responsiblePerson.toLowerCase().includes(q)) ||
        (t.notes && t.notes.toLowerCase().includes(q))
      );
    }
    return list;
  }, [tasks, taskSearchQuery, taskTrackingDate]);
  ...
  const filteredCommittees = useMemo(() => {
    if (!committeeSearchQuery.trim()) return committees;
    const q = committeeSearchQuery.toLowerCase().trim();
  ...
  }, [committees, committeeSearchQuery]);
  ...
  const [activeMissionType, setActiveMissionType] = useState<'mission' | 'leave'>('mission');
  const [filterMode, setFilterMode] = useState<'all' | 'mission' | 'leave'>('all');
  const [showReminders, setShowReminders] = useState(false);
  const [deletingOfficialId, setDeletingOfficialId] = useState<string | null>(null);
  const [deletingTaskId, setDeletingTaskId] = useState<string | null>(null);
  const [deletingCommitteeId, setDeletingCommitteeId] = useState<string | string[] | null>(null);
  ...
  const logAction = async (action: 'CREATE' | 'UPDATE' | 'DELETE', collectionName: string, documentId: string, details: string) => {
    if (!user) return;
    try {
  ...
  const handleAppError = (err: unknown, operation: OperationType, path: string | null) => {
    try {
  ...
  const uploadFileToFirestore = async (file: File): Promise<string> => {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      chunkPromises.push(
        addDoc(collection(db, 'file_chunks'), {
          fileId: fileId,
          chunkIndex: i,
          data: chunkData
        }).then(() => console.log(`Uploaded chunk ${i + 1}/${totalChunks}`))
      );
    }
  ...
  const reminders = useMemo(() => {
    ...
    tasks.forEach(task => {
        if (task.status === '✅ បញ្ចប់រួចរាល់' || !task.deadline) return;

        const deadlineDate = startOfDay(new Date(task.deadline));
        const diffInDays = differenceInCalendarDays(deadlineDate, today);

        if (task.documentType === 'meeting') {
            // "ប្រជុំ" (meeting): 1 day before
            if (diffInDays === 1) {
                list.push({
                    id: `meeting-warn-${task.id}`,
                    type: 'upcoming',
                    officialName: task.activity,
                    message: `ប្រភេទលិខិត "ប្រជុំ"៖ នឹងផុតកំណត់នៅថ្ងៃស្អែក (${task.deadline})`,
                    severity: 'warning'
                });
            } else if (diffInDays === 0) {
                list.push({
                    id: `meeting-now-${task.id}`,
                    type: 'urgent',
                    officialName: task.activity,
                    message: `ប្រភេទលិខិត "ប្រជុំ"៖ ផុតកំណត់នៅថ្ងៃនេះ (${task.deadline})`,
                    severity: 'error'
                });
            } else if (diffInDays < 0) {
                list.push({
                    id: `meeting-over-${task.id}`,
                    type: 'overdue',
                    officialName: task.activity,
                    message: `ប្រភេទលិខិត "ប្រជុំ"៖ ហួសថ្ងៃផុតកំណត់ (${task.deadline})`,
                    severity: 'error'
                });
            }
        } else if (task.documentType === 'feedback') {
            // "ផ្តល់មតិ" (feedback): 3 days before
            if (diffInDays === 3) {
                list.push({
                    id: `feedback-warn-${task.id}`,
                    type: 'upcoming',
                    officialName: task.activity,
                    message: `ប្រភេទលិខិត "ផ្តល់មតិ"៖ នៅសល់ ៣ ថ្ងៃទៀត នឹងផុតកំណត់ (${task.deadline})`,
                    severity: 'info'
                });
            } else if (diffInDays > 0 && diffInDays < 3) {
                list.push({
                    id: `feedback-urgent-${task.id}`,
                    type: 'urgent',
                    officialName: task.activity,
                    message: `ប្រភេទលិខិត "ផ្តល់មតិ"៖ ជិតដល់ថ្ងៃផុតកំណត់ហើយ (នៅសល់ ${diffInDays} ថ្ងៃ)`,
                    severity: 'warning'
                });
            } else if (diffInDays === 0) {
                list.push({
                    id: `feedback-now-${task.id}`,
                    type: 'urgent',
                    officialName: task.activity,
                    message: `ប្រភេទលិខិត "ផ្តល់មតិ"៖ ផុតកំណត់នៅថ្ងៃនេះ (${task.deadline})`,
                    severity: 'error'
                });
            } else if (diffInDays < 0) {
                list.push({
                    id: `feedback-over-${task.id}`,
                    type: 'overdue',
                    officialName: task.activity,
                    message: `ប្រភេទលិខិត "ផ្តល់មតិ"៖ ហួសថ្ងៃផុតកំណត់ (${task.deadline})`,
                    severity: 'error'
                });
            }
        }
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logAction('CREATE', 'users', newUser.uid, `បង្កើតអ្នកប្រើប្រាស់ថ្មី៖ ${newUserName} (${newUserEmail}) ដែលមានតួនាទីជា ${newUserRole}`);
  ...
  useEffect(() => {
    ...
    }, (err) => handleAppError(err, OperationType.LIST, 'groups'));
    ...
    const unsubscribeOfficials = onSnapshot(officialsQuery, (snapshot) => {
      ...
      setOfficials(data);
    }, (err) => handleAppError(err, OperationType.LIST, 'officials'));
    ...
    const unsubscribeMissions = onSnapshot(missionsQuery, (snapshot) => {
      ...
      setMissions(data);
    }, (err) => handleAppError(err, OperationType.LIST, 'missions'));
    ...
    const unsubscribeCommittees = onSnapshot(committeesQuery, (snapshot) => {
      ...
      setCommittees(data);
    }, (err) => handleAppError(err, OperationType.LIST, 'committees'));

    const tasksQuery = query(collection(db, 'tasks'), orderBy('order', 'asc'));
    const unsubscribeTasks = onSnapshot(tasksQuery, (snapshot) => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as TaskTracking));
      setTasks(data);
    }, (err) => handleAppError(err, OperationType.LIST, 'tasks'));
    ...
    if (isAdmin) {
      ...
      unsubscribeAllUsers = onSnapshot(usersQuery, (snapshot) => {
        ...
        setAllUsers(data);
      }, (err) => handleAppError(err, OperationType.LIST, 'users'));
    }
  ...
  const handleAddGroup = async () => {
    ...
    try {
      ...
      logAction('CREATE', 'groups', docRef.id, `បន្ថែមក្រុមថ្មី៖ ${newGroupName.trim()}`);
      setNewGroupName('');
      showToast('បានបន្ថែមក្រុមថ្មីដោយជោគជ័យ');
    } catch (err) {
      handleAppError(err, OperationType.WRITE, 'groups');
    }
  };
  ...
  const handleEditGroup = async () => {
    ...
    try {
      ...
      logAction('UPDATE', 'groups', editingGroup.id, `កែសម្រួលឈ្មោះក្រុមពី "${editingGroup.name}" ទៅជា "${newGroupName.trim()}"`);
  ...
  const handleDeleteGroup = async (id: string) => {
    if (!isAdmin) return;
    try {
      const deletedGroupName = groups.find(g => g.id === id)?.name || id;
      await deleteDoc(doc(db, 'groups', id));
      logAction('DELETE', 'groups', id, `លុបក្រុម៖ ${deletedGroupName}`);
      setDeletingGroupId(null);
      showToast('បានលុបក្រុមដោយជោគជ័យ');
    } catch (err) {
      handleAppError(err, OperationType.DELETE, 'groups');
    }
  };
  ...
  const handleAddCommittee = async () => {
    ...
    try {
      ...
      logAction('CREATE', 'committees', 'multiple', `បន្ថែមព័ត៌មានថ្មីសម្រាប់ថ្នាក់ដឹកនាំ៖ ${newCommittee.leaderName}`);
      ...
      handleAppError(err, OperationType.WRITE, 'committees');
    } finally {
  ...
  const handleEditCommittee = async () => {
    ...
    try {
      ...
      logAction('UPDATE', 'committees', editingCommittee.leaderName, `កែសម្រួលព័ត៌មានថ្នាក់ដឹកនាំ៖ ${editingCommittee.leaderName}`);
  ...
  const handleDeleteCommittee = async (ids: string | string[]) => {
    ...
    try {
      ...
      handleAppError(err, OperationType.DELETE, 'committees');
    }
  };
  ...
  const handleAddOfficial = async () => {
    ...
    try {
      ...
      logAction('CREATE', 'officials', docRef.id, `បន្ថែមមន្ត្រីថ្មី៖ ${newOfficial.name} (${newOfficial.position})`);
      setNewOfficial({ name: '', position: '', group: 'General Affairs', groupDescription: '', gender: 'M' });
      setIsAddingOfficial(false);
      setError(null);
    } catch (err) {
      handleAppError(err, OperationType.CREATE, 'officials');
    }
  };
  ...
  const handleBulkImport = async () => {
    ...
    if (importFile) {
      try {
        const ExcelJSNamespace = await import('exceljs');
        ...
        if (worksheet) {
          worksheet.eachRow((row, rowNumber) => {
            const rowValues = Array.isArray(row.values) ? row.values.slice(1) : []; // ExcelJS row values are 1-indexed
  ...
  const handleDeleteOfficial = async (id: string) => {
    if (!isEditor) return;
    try {
      const deletedName = officials.find(o => o.id === id)?.name || id;
      await deleteDoc(doc(db, 'officials', id));
      logAction('DELETE', 'officials', id, `លុបមន្ត្រី៖ ${deletedName}`);
      ...
      handleAppError(err, OperationType.DELETE, 'officials');
    }
  };
  ...
  const toggleMissionDay = async (officialId: string, day: number) => {
    ...
    try {
      if (existingMission) {
        ...
        const allLeaves = Array.from(new Set(matchingMissions.flatMap(m => m.leaves || [])));
        const allNotes = matchingMissions.reduce((acc, m) => ({ ...acc, ...(m.notes || {}) }), {} as Record<string, string>);
        
        let newDays = [...allDays];
        let newLeaves = [...allLeaves];

        if (activeMissionType === 'mission') {
          if (newDays.includes(day)) {
            newDays = newDays.filter(d => d !== day);
          } else {
            newDays = [...newDays, day];
            newLeaves = newLeaves.filter(d => d !== day); // Remove from leaves if adding to days
          }
        } else {
          if (newLeaves.includes(day)) {
            newLeaves = newLeaves.filter(d => d !== day);
          } else {
            newLeaves = [...newLeaves, day];
            newDays = newDays.filter(d => d !== day); // Remove from days if adding to leaves
          }
        }
        ...
        const officialName = officials.find(o => o.id === officialId)?.name || 'Unknown';
        logAction('UPDATE', 'missions', missionId, `ធ្វើបច្ចុប្បន្នភាពបេសកកម្មសម្រាប់ ${officialName}: ថ្ងៃ=${newDays.join(',')}, ច្បាប់=${newLeaves.join(',')}`);
        ...
        await setDoc(doc(db, 'missions', missionId), {
          ...
          month,
          year,
          days: activeMissionType === 'mission' ? [day] : [],
          leaves: activeMissionType === 'leave' ? [day] : [],
          notes: {}
        });
      }
      ...
      handleAppError(err, OperationType.WRITE, 'missions');
    }
  };
  ...
  const deleteMissionDay = async (officialId: string, day: number) => {
    ...
    try {
      ...
      const newLeaves = allLeaves.filter(d => d !== day);
      const newNotes = { ...allNotes };
      delete newNotes[day.toString()];

      if (newDays.length === 0 && newLeaves.length === 0) {
        ...
        const officialName = officials.find(o => o.id === officialId)?.name || 'Unknown';
        logAction('DELETE', 'missions', missionId, `លុបបេសកកម្មទាំងអស់សម្រាប់ ${officialName}`);
      } else {
        ...
        const officialName = officials.find(o => o.id === officialId)?.name || 'Unknown';
        logAction('UPDATE', 'missions', missionId, `លុបថ្ងៃទី ${day} ចេញពីបេសកកម្មរបស់ ${officialName}`);
      ...
      handleAppError(err, OperationType.DELETE, 'missions');
    } finally {
  ...
  const updateMissionNote = async () => {
    ...
    try {
      ...
      showToast('បរាជ័យក្នុងការរក្សាទុកកត់ចំណាំ', 'error');
      handleAppError(err, OperationType.WRITE, 'missions');
    } finally {
  ...
  const toggleDayForSelected = async (day: number) => {
    ...
    const allHaveTarget = selectedOfficialIds.every(id => {
      const m = selectedMissions.find(sm => sm.officialId === id);
      return activeMissionType === 'mission' 
        ? m?.days.includes(day)
        : m?.leaves?.includes(day);
    });

    try {
      const chunks = [];
      for (let i = 0; i < selectedOfficialIds.length; i += 450) {
        chunks.push(selectedOfficialIds.slice(i, i + 450));
      }

      for (const chunk of chunks) {
        const batch = writeBatch(db);
        chunk.forEach((officialId) => {
          const missionId = `${officialId}_${month}_${year}`;
          const existingMission = selectedMissions.find(m => m.officialId === officialId);
          
          if (existingMission) {
            let newDays = [...(existingMission.days || [])];
            let newLeaves = [...(existingMission.leaves || [])];

            if (activeMissionType === 'mission') {
              if (allHaveTarget) {
                newDays = newDays.filter(d => d !== day);
              } else {
                newDays = Array.from(new Set([...newDays, day]));
                newLeaves = newLeaves.filter(d => d !== day);
              }
            } else {
              if (allHaveTarget) {
                ...
                newLeaves = Array.from(new Set([...newLeaves, day]));
                newDays = newDays.filter(d => d !== day);
              }
            }
            
            batch.update(doc(db, 'missions', existingMission.id), {
              days: newDays,
              leaves: newLeaves
            });
      ...
      showToast(allHaveTarget 
        ? (activeMissionType === 'mission' ? 'បានលុបបេសកកម្មជាក្រុម' : 'បានលុបច្បាប់ជាក្រុម')
        : (activeMissionType === 'mission' ? 'បានបន្ថែមបេសកកម្មជាក្រុម' : 'បានបន្ថែមច្បាប់ជាក្រុម')
      );
    } catch (err) {
  ...
  const handleBulkUpdateNotes = async () => {
    ...
    try {
      const idChunks = [];
      for (let i = 0; i < selectedOfficialIds.length; i += 450) {
        idChunks.push(selectedOfficialIds.slice(i, i + 450));
      }

      for (const chunk of idChunks) {
        const batch = writeBatch(db);
        for (const officialId of chunk) {
          ...
          if (existingMission) {
            ...
            let newDays = Array.from(new Set([...(existingMission.days || [])]));
            let newLeaves = Array.from(new Set([...(existingMission.leaves || [])]));
            
            if (bulkUpdateType === 'mission') {
              newDays = Array.from(new Set([...newDays, ...selectedDays]));
              newLeaves = newLeaves.filter(day => !selectedDays.includes(day));
            } else if (bulkUpdateType === 'leave') {
              newLeaves = Array.from(new Set([...newLeaves, ...selectedDays]));
              newDays = newDays.filter(day => !selectedDays.includes(day));
            } else {
              newDays = Array.from(new Set([...newDays, ...selectedDays]));
            }
            ...
            batch.set(doc(db, 'missions', missionId), {
              ...
              month,
              year,
              days: bulkUpdateType === 'leave' ? [] : selectedDays,
              leaves: bulkUpdateType === 'leave' ? selectedDays : [],
  ...
  const handleUpdateOfficial = async () => {
    ...
    try {
      ...
      logAction('UPDATE', 'officials', editingOfficial.id, `កែសម្រួលព័ត៌មានមន្ត្រី៖ ${editingOfficial.name}`);
      setEditingOfficial(null);
    } catch (err) {
      handleAppError(err, OperationType.WRITE, 'officials');
    }
  };
  ...
  const handleAddTask = async () => {
    if (!isEditor || !newTask.activity) return;
    try {
      ...
      logAction('CREATE', 'tasks', docRef.id, `បន្ថែមសកម្មភាពថ្មី៖ ${newTask.activity}`);
      setNewTask({
        activity: '',
        status: '⚠️ មិនទាន់ចាប់ផ្តើម',
        startDate: format(new Date(), 'yyyy-MM-dd'),
      ...
      setSuccessMessage('បន្ថែមសកម្មភាពបានជោគជ័យ!');
      setTimeout(() => setSuccessMessage(null), 3000);
    } catch (err) {
      handleAppError(err, OperationType.CREATE, 'tasks');
    }
  };

  const handleUpdateTask = async () => {
    if (!isEditor || !editingTask) return;
    try {
      const { id, ...data } = editingTask;
      await updateDoc(doc(db, 'tasks', id), data);
      logAction('UPDATE', 'tasks', id, `កែសម្រួលសកម្មភាព៖ ${editingTask.activity}`);
      setEditingTask(null);
      setSuccessMessage('កែសម្រួលបានជោគជ័យ!');
      setTimeout(() => setSuccessMessage(null), 3000);
    } catch (err) {
      handleAppError(err, OperationType.WRITE, 'tasks');
    }
  };
  ...
  const handleDeleteTask = async (id: string) => {
    if (!isEditor) return;
    try {
      const deletedTaskActivity = tasks.find(t => t.id === id)?.activity || id;
      await deleteDoc(doc(db, 'tasks', id));
      logAction('DELETE', 'tasks', id, `លុបសកម្មភាព៖ ${deletedTaskActivity}`);
      setDeletingTaskId(null);
    } catch (err) {
      handleAppError(err, OperationType.DELETE, 'tasks');
    }
  };
  ...
  const updateUserRole = async (uid: string, role: 'admin' | 'editor' | 'viewer') => {
    ...
    try {
      ...
      logAction('UPDATE', 'users', uid, `កែសម្រួលសិទ្ធិអ្នកប្រើប្រាស់ទៅជា៖ ${role}`);
    } catch (err) {
      handleAppError(err, OperationType.WRITE, `users/${uid}`);
  ...
  const filteredOfficials = useMemo(() => {
    ...
    if (filterDateRange.start !== null && filterDateRange.end !== null) {
      result = result.filter(o => {
        ...
        return mission.days.some(day => day >= filterDateRange.start! && day <= filterDateRange.end!) ||
               (mission.leaves && mission.leaves.some(day => day >= filterDateRange.start! && day <= filterDateRange.end!));
      });
    }
    ...
    if (filterMode !== 'all') {
      result = result.filter(o => {
        ...
        const mission = missions.find(m => m.officialId === o.id && m.month === month && m.year === year);
        if (!mission) return false;
        if (filterMode === 'mission') return mission.days.length > 0;
        if (filterMode === 'leave') return mission.leaves && mission.leaves.length > 0;
        return true;
      });
    }
    ...
    return result;
  }, [officials, searchQuery, selectedGroup, filterDateRange, missions, filterMode]);
  ...
  const officialsForExportBase = useMemo(() => {
    ...
    return result;
  }, [officials, searchQuery, filterDateRange, missions, filterMode]);

  useEffect(() => {
    setSelectedGroup('all');
  }, [activeTab]);

  const handleExportToExcel = async (config = exportConfig) => {
    const month = getMonth(currentDate) + 1;
    const year = getYear(currentDate);
    ...
    if (config.onlyWithMissions) {
      officialsToExport = officialsToExport.filter(o => {
        const mission = missions.find(m => m.officialId === o.id && m.month === month && m.year === year);
        return mission && (mission.days.length > 0 || (mission.leaves && mission.leaves.length > 0));
      });
    }
    ...
    // Set Column Widths
    const columns = [
      { header: 'ល.រ', key: 'no', width: 5 },
      { header: 'ឈ្មោះ', key: 'name', width: 25 },
      { header: 'តួនាទី', key: 'position', width: 25 },
      { header: 'ក្រុម', key: 'group', width: 20 },
      { header: 'ភេទ', key: 'gender', width: 8 }
    ];

    if (config.columns.days) {
      for (let day = 1; day <= daysInMonth; day++) {
        columns.push({ header: day.toString(), key: `day${day}`, width: 4 });
      }
    }
    ...
    // Header styling
    const headerFont = { name: 'Khmer OS Battambang', size: 12, bold: true };
    const subHeaderFont = { name: 'Khmer OS Battambang', size: 11 };
    const normalFont = { name: 'Khmer OS Battambang', size: 10 };

    // Add Title and Subtitle rows
    worksheet.insertRow(1, []);
    worksheet.insertRow(2, [`បញ្ជីបេសកកម្មការិយាល័យរដ្ឋបាល`]);
    worksheet.insertRow(3, [`សម្រាប់ខែ ${monthNames[month - 1]} ឆ្នាំ ${year}`]);
    ...
    worksheet.getRow(2).getCell(1).font = { ...headerFont, size: 16, color: { argb: 'FF1E40AF' } };
    worksheet.getRow(2).getCell(1).alignment = { horizontal: 'center', vertical: 'middle' };

    // Style Subtitle
    worksheet.getRow(3).height = 25;
    worksheet.getRow(3).getCell(1).font = subHeaderFont;
    worksheet.getRow(3).getCell(1).alignment = { horizontal: 'center', vertical: 'middle' };

    // Header Row (Now on Row 5)
    const headerRow = worksheet.getRow(5);
    headerRow.values = columns.map(c => c.header);
    headerRow.font = { ...headerFont, size: 10 };
    headerRow.height = 25;
    ...
    // Add Data
    officialsToExport.forEach((official, idx) => {
      const mission = missions.find(m => m.officialId === official.id && m.month === month && m.year === year);
      const rowData: any = {
        no: idx + 1,
        name: official.name,
        position: official.position,
        group: getGroupNameKh(official.group),
        gender: official.gender === 'M' ? 'ប' : official.gender === 'F' ? 'ស' : '-'
      };

      if (config.columns.days) {
        for (let day = 1; day <= daysInMonth; day++) {
          const isOnMission = mission?.days.includes(day);
          const isLeave = mission?.leaves?.includes(day);
      ...
      row.eachCell((cell, colNumber) => {
        ...
        // Alignment
        if (colNumber === 1 || colNumber === 5 || colNumber > 5) {
          cell.alignment = { horizontal: 'center', vertical: 'middle' };
        } else {
        ...
        // Color 'X' mark
        if (colNumber > 5 && cell.value === 'X') {
          cell.font = { ...normalFont, color: { argb: 'FFDC2626' }, bold: true };
        }
      });
    });
    ...
    saveAs(blob, `Missions_Export_${month}_${year}.xlsx`);
              ...
              return `
                <tr>
                  ...
                  ${config.columns.days ? Array.from({ length: daysInMonth }, (_, i) => {
                    ...
                    if (isMission) {
                      content = 'X';
                      className = 'mission-day';
                    } else if (isLeave) {
                      content = 'L';
                      className = 'leave-day';
                    }
                    return `<td class="text-center day-cell ${className}">${content}</td>`;
                  }).join('') : ''}
                </tr>
              `;
            }).join('')}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans transition-colors duration-300">
      ...
      <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-[100]">
        <div className="max-w-[1600px] mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg leading-tight dark:text-white">ប្រព័ន្ធគ្រប់គ្រងទិន្នន័យ</h1>
              <p className="text-xs text-gray-500 dark:text-gray-400">ការិយាល័យរដ្ឋបាល</p>
            </div>
          </div>
      ...
      <main className="max-w-[1600px] mx-auto p-4 md:p-6">
        {mainView === 'missions' ? (
          ...
          <div className="flex items-center gap-2 bg-white dark:bg-gray-900 p-1 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 shrink-0">
            <button 
              ...
              <ChevronLeft className="w-5 h-5" />
                ...
                <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                  <span className="text-[11px] font-bold text-blue-600 dark:text-blue-400 whitespace-nowrap">
                    {format(currentDate, 'MMMM yyyy', { locale: km }).replace('ខែ', '').replace('ឆ្នាំ', '')}
                  </span>
                </div>
            <button 
            ...
            <button 
              ...
              className="px-2 py-1 text-[10px] font-bold text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors border border-blue-100 dark:border-blue-900/50"
            >
              ថ្ងៃនេះ
            </button>
          </div>
          ...
          <div className="flex-1 w-full">
            <div className="relative group w-full">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/20 focus:border-blue-500 outline-none transition-all shadow-sm hover:border-gray-300 dark:hover:border-gray-700 focus:shadow-md text-base dark:text-white"
              />
            </div>
          </div>
          
          <div className="flex items-center gap-2 shrink-0">
            {mainView === 'missions' && filterDateRange.start !== null && (
              ...
              <div className="relative min-w-[120px]">
                <select
                  ...
                  className="w-full appearance-none pl-3 pr-8 py-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl focus:ring-4 focus:ring-brand-light dark:focus:ring-brand/20 focus:border-brand outline-none transition-all shadow-sm text-xs font-medium text-gray-700 dark:text-gray-300 cursor-pointer hover:border-gray-300 dark:hover:border-gray-700"
                >
              ...
              <div className="relative min-w-[100px]">
                <select
                  ...
                  className="w-full appearance-none pl-3 pr-8 py-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/20 focus:border-blue-500 outline-none transition-all shadow-sm text-xs font-medium text-gray-700 dark:text-gray-300 cursor-pointer hover:border-gray-300 dark:hover:border-gray-700"
                >
              ...
              <div className="flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-1 shadow-sm shrink-0">
                <button
        ...
        {successMessage && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-8 left-1/2 -translate-x-1/2 z-[300] bg-emerald-500 text-white px-6 py-3 rounded-2xl shadow-xl font-bold flex items-center gap-3"
          >
          ...
          {viewMode === 'dashboard' ? (
            ...
            >
                    ...
                    <table className="w-full border-collapse text-sm border-hidden">
                      <thead className="sticky top-0 z-30">
                        <tr className="bg-gray-50/80 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-800">
                          <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 w-12 sticky left-0 top-0 bg-gray-50 dark:bg-gray-800 z-40 border border-gray-200 dark:border-gray-700">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px]">ល.រ</span>
                            </div>
                          </th>
                        <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 min-w-[200px] sticky left-12 top-0 bg-gray-50 dark:bg-gray-800 z-40 border border-gray-200 dark:border-gray-700">
                          <div className="flex flex-col gap-1">
                            <span className="text-[10px] uppercase tracking-wider">ឈ្មោះ / តួនាទី</span>
                          </div>
                        </th>
                          <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-16 sticky top-0 bg-gray-50 dark:bg-gray-800 z-30 border border-gray-200 dark:border-gray-700 text-[10px]">ភេទ</th>
                          {daysArray.map(day => {
                            ...
                            return (
                              <th 
                                ...
                                className={cn(
                                  "p-2 text-center font-bold text-gray-600 dark:text-gray-400 w-10 sticky top-0 bg-gray-50 dark:bg-gray-800 z-30 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors border border-gray-200 dark:border-gray-700 text-[10px]",
                          ...
                          {isEditor && <th className="p-4 text-center font-bold text-gray-600 w-16 sticky top-0 bg-gray-50 dark:bg-gray-800 z-30 border border-gray-200 dark:border-gray-700 text-[10px]">លុប</th>}
                        </tr>
                      </thead>
                      <tbody>
                        {leadershipOfficials.map((official, idx) => {
                          const month = getMonth(currentDate) + 1;
                              const year = getYear(currentDate);
                              const matchingMissions = missions.filter(m => m.officialId === official.id && m.month === month && m.year === year);
                              const allDays = Array.from(new Set(matchingMissions.flatMap(m => m.days || [])));
                              const allLeaves = Array.from(new Set(matchingMissions.flatMap(m => m.leaves || [])));
                              const allNotes = matchingMissions.reduce((acc, m) => ({ ...acc, ...(m.notes || {}) }), {} as Record<string, string>);
                              return (
                                <tr key={official.id} className={cn(
                                  ...
                                  <td className="p-4 text-gray-500 dark:text-gray-400 sticky left-0 bg-white dark:bg-gray-900 group-hover:bg-gray-50 dark:group-hover:bg-gray-800 z-20 transition-colors border border-gray-200 dark:border-gray-700 text-xs font-mono font-bold text-center">
                                    <div className="flex items-center gap-2">
                                      {isEditor && (
                                        <input 
                                          type="checkbox" 
                                  ...
                                  <td 
                                    className="p-4 sticky left-12 bg-white dark:bg-gray-900 group-hover:bg-gray-50 dark:group-hover:bg-gray-800 z-20 cursor-pointer transition-colors border border-gray-200 dark:border-gray-700"
                                    onDoubleClick={() => isEditor && setEditingOfficial(official)}
                                    title={isEditor ? "ចុចពីរដងដើម្បីកែសម្រួល" : ""}
                                  >
                                    <div className="font-bold text-gray-900 dark:text-gray-100 text-xs">{official.name}</div>
                                    <div className="text-[10px] text-gray-500 dark:text-gray-400 font-medium">{official.position}</div>
                                  </td>
                              <td className="p-4 text-center text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 font-bold text-xs">
                                {official.gender === 'M' ? 'ប' : official.gender === 'F' ? 'ស' : '-'}
                              </td>
                              {daysArray.map(day => {
                                const isWeekend = [0, 6].includes(new Date(getYear(currentDate), getMonth(currentDate), day).getDay());
                                const isOnMission = allDays.includes(day);
                                const isLeave = allLeaves.includes(day);
                                const note = allNotes[day.toString()];
                                return (
                                  <td 
                                    key={day} 
                                    className={cn(
                                      "p-0 border border-gray-200 dark:border-gray-700 transition-all cursor-pointer relative group/cell",
                                      isWeekend && "bg-orange-50/20 dark:bg-orange-900/5",
                                      isOnMission && !note && "bg-red-50/40 dark:bg-red-900/20",
                                      isOnMission && note && "bg-blue-50/40 dark:bg-blue-900/20",
                                      isLeave && "bg-yellow-50/40 dark:bg-yellow-900/20",
                              ...
                              {isEditor && (
                                <td className="p-4 text-center border border-gray-200 dark:border-gray-700">
                                  <button 
              ...
              {(activeTab === 'all' || activeTab === 'officials') && (
                <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-brand-light/30 dark:bg-brand/10">
                  <h3 className="font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                    <div className="w-2 h-6 bg-brand rounded-full" />
                    នាយកដ្ឋានរដ្ឋបាល
                  </h3>
                  ...
                  <table className="w-full border-collapse text-sm border-hidden">
                    <thead className="sticky top-0 z-30">
                      <tr className="bg-gray-50/80 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-800">
                        ...
                        <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 min-w-[200px] sticky left-12 top-0 bg-gray-50 dark:bg-gray-800 z-40 border border-gray-200 dark:border-gray-700">
                          <div className="flex flex-col gap-1">
                            <span className="text-[10px] uppercase tracking-wider">ឈ្មោះ / តួនាទី / ក្រុម</span>
                          </div>
                        </th>
                        <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-16 sticky top-0 bg-gray-50 dark:bg-gray-800 z-30 border border-gray-200 dark:border-gray-700 text-[10px]">ភេទ</th>
                        {daysArray.map(day => {
                          ...
                          return (
                            <th 
                              ...
                              className={cn(
                                "p-2 text-center font-bold text-gray-600 dark:text-gray-400 w-10 border border-gray-200 dark:border-gray-700 sticky top-0 bg-gray-50 dark:bg-gray-800 z-30 cursor-pointer hover:bg-brand-light dark:hover:bg-brand/20 transition-colors text-[10px]",
                                isWeekend && "bg-orange-50/50 dark:bg-orange-900/10",
                        ...
                        {isEditor && <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-16 sticky top-0 bg-gray-50 dark:bg-gray-800 z-30 border border-gray-200 dark:border-gray-700 text-[10px]">លុប</th>}
                      </tr>
                    </thead>
                    <tbody>
                      {otherGroups.length === 0 && leadershipOfficials.length === 0 && (
                        <tr>
                          <td colSpan={daysInMonth + 4} className="p-12 text-center text-gray-500 dark:text-gray-400 italic border border-gray-200 dark:border-gray-700">
                      ...
                      {otherGroups.map(groupName => (
                        <React.Fragment key={groupName}>
                          <tr className="bg-gray-100/80 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700">
                            <td colSpan={daysInMonth + 4} className="p-3 font-bold text-brand-hover dark:text-brand-light uppercase tracking-wide text-xs border border-gray-200 dark:border-gray-700">
                          ...
                          {otherOfficials
                            ...
                            .map((official, idx) => {
                              ...
                              const year = getYear(currentDate);
                                  const matchingMissions = missions.filter(m => m.officialId === official.id && m.month === month && m.year === year);
                                  const mission = matchingMissions[0];
                                  const allDays = Array.from(new Set(matchingMissions.flatMap(m => m.days || [])));
                                  const allLeaves = Array.from(new Set(matchingMissions.flatMap(m => m.leaves || [])));
                                  const allNotes = matchingMissions.reduce((acc, m) => ({ ...acc, ...(m.notes || {}) }), {} as Record<string, string>);
                                  return (
                                  ...
                                  {daysArray.map(day => {
                                      ...
                                      <td 
                                        ...
                                        className={cn(
                                          ...
                                          isLeave && "bg-yellow-50/40 dark:bg-yellow-900/20 border-yellow-200/50 dark:border-yellow-800/50",
                                          !isEditor && "cursor-default"
                                        )}
                                        onClick={() => toggleMissionDay(official.id, day)}
                                      >
              ...
              <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-800 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                ...
                {(() => {
                  ...
                  return calendarDays.map(day => {
                    ...
                    const dayLeaves = missions.filter(m => m.leaves?.includes(day.getDate()) && isSameMonth(day, currentDate));
                    const isCurrentMonth = isSameMonth(day, currentDate);
                    const isWeekend = [0, 6].includes(day.getDay());
                    const hasAnyNote = dayMissions.some(m => m.notes?.[day.getDate().toString()]);
                    const hasAnyLeave = dayLeaves.length > 0;
                    ...
                    return (
                      <div 
                        ...
                        className={cn(
                          ...
                          (dayMissions.length > 0 || hasAnyLeave) && isCurrentMonth && (
                            hasAnyLeave
                              ? "bg-yellow-50/40 dark:bg-yellow-900/20 border-yellow-200/50 dark:border-yellow-800/50 shadow-inner"
                              : hasAnyNote 
                          ...
                          dayMissions.length === 0 && !hasAnyLeave && isCurrentMonth && "border-transparent"
                        )}
                      >
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {isCurrentMonth && (dayMissions.length > 0 || hasAnyLeave) && (
                            hasAnyLeave ? (
                              <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 p-1 rounded-md shadow-sm">
                        ...
                        <div className="flex flex-col gap-1 overflow-y-auto max-h-[100px] scrollbar-hide">
                          {[...dayMissions, ...dayLeaves].map(m => {
                            const official = officials.find(o => o.id === m.officialId);
                            if (!official) return null;
                            const note = m.notes?.[day.getDate().toString()];
                            const isMissionsLeave = m.leaves?.includes(day.getDate());
                            return (
                              <div 
                                key={m.id} 
                                className={cn(
                                ...
                                title={note ? note.replace(/<(.|\n)*?>/g, '') : isMissionsLeave ? 'ច្បាប់' : official.name}
                              >
                                <span className="truncate font-medium">{official.name}</span>
                                {isMissionsLeave ? (
                                  <AlertCircle className="w-2.5 h-2.5 text-yellow-500 shrink-0" />
                                ) : note ? (
                                  <MessageSquare className="w-2.5 h-2.5 text-blue-500 fill-blue-500 shrink-0" />
                                ) : (
        ...
        <div className="mt-8 flex flex-wrap items-center gap-6 text-sm text-gray-500 bg-white p-4 rounded-xl border border-gray-200 shadow-sm w-fit">
          ...
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-yellow-50 border border-yellow-200 rounded flex items-center justify-center shadow-sm">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex items-center gap-4">
                <div className="relative w-72">
                  ...
                  <input
                    type="text"
                    placeholder="ស្វែងរកឈ្មោះ លេខលិខិត ឬកម្មវត្ថុ..."
                    value={committeeSearchQuery}
                    onChange={(e) => setCommitteeSearchQuery(e.target.value)}
                    className="block w-full pl-9 pr-8 py-2.5 border border-gray-200 dark:border-gray-800 rounded-xl leading-5 bg-white dark:bg-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 sm:text-sm transition-all shadow-sm"
                  />
                  {committeeSearchQuery && (
                    <button
                      onClick={() => setCommitteeSearchQuery('')}
                      className="absolute inset-y-0 right-0 pr-3 flex items-center hover:text-red-500 text-gray-400 transition-colors"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  )}
                ...
                {isEditor && (
                  <button
                    ...
                    className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-emerald-100 dark:shadow-none hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0"
                  >
            ...
            <CommitteesTable 
              ...
              onDelete={setDeletingCommitteeId}
              isEditor={isEditor}
          ...
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 bg-brand-light dark:bg-brand/10 rounded-2xl flex items-center justify-center shadow-inner ring-1 ring-brand/10">
                  <ClipboardCheck className="text-brand w-8 h-8" />
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">តាមដានសកម្មភាពការងារ</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">តាមដានស្ថានភាពសកម្មភាពការងារ និងការទទួលខុសត្រូវ</p>
                </div>
              </div>
              <div className="flex flex-col md:flex-row items-center gap-4">
                {/* Month/Year Navigation for Tasks */}
                <div className="flex items-center bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-1.5">
                  <button 
                    onClick={() => setTaskTrackingDate(addMonths(taskTrackingDate, -1))}
                    className="p-2.5 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-xl transition-all active:scale-90"
                  >
                    <ChevronLeft className="w-5 h-5 text-gray-600 dark:text-gray-400" />
                  </button>
                  <div className="px-6 py-1.5 bg-brand-light/20 dark:bg-brand/10 rounded-xl">
                    <span className="text-sm font-bold text-brand italic">
                      {format(taskTrackingDate, 'MMMM yyyy', { locale: km })}
                    </span>
                  </div>
                  <button 
                    onClick={() => setTaskTrackingDate(addMonths(taskTrackingDate, 1))}
                ...
                <div className="relative w-full md:w-80">
                  ...
                  <input
                    type="text"
                    placeholder="ស្វែងរកសកម្មភាព ឬអ្នកទទួលខុសត្រូវ..."
                    value={taskSearchQuery}
                    onChange={(e) => setTaskSearchQuery(e.target.value)}
                    className="block w-full pl-11 pr-10 py-3 border border-gray-200 dark:border-gray-800 rounded-2xl bg-white dark:bg-gray-900 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-brand/10 focus:border-brand sm:text-sm transition-all shadow-sm"
                  />
                  {taskSearchQuery && (
                    <button
                ...
                {isEditor && (
                  <button
                    ...
                    className="flex items-center gap-2 bg-brand hover:bg-brand-hover text-white px-6 py-3 rounded-2xl font-bold transition-all shadow-lg shadow-brand/10 hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 whitespace-nowrap"
                  >
                    <Plus className="w-5 h-5" />
                    បន្ថែមសកម្មភាព
                  </button>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full border-collapse text-sm border-hidden">
                        <thead>
                            <tr className="bg-gray-50/80 dark:bg-gray-800/80">
                                <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-16 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">ល.រ</th>
                                <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 min-w-[350px] uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">ឈ្មោះសកម្មភាព</th>
                                <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-32 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">ប្រភេទលិខិត</th>
                                <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-24 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">ឯកសារ</th>
                                <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-32 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">ស្ថានភាព</th>
                                <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-32 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">ថ្ងៃចាប់ផ្ដើម</th>
                                <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-32 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">ថ្ងៃផុតកំណត់</th>
                                <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 w-48 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">អ្នកទទួលខុសត្រូវ</th>
                                <th className="p-4 text-left font-bold text-gray-600 dark:text-gray-400 w-32 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700">ផ្សេងៗ</th>
                                {isEditor && <th className="p-4 text-center font-bold text-gray-600 dark:text-gray-400 w-24 sticky right-0 bg-gray-50/90 dark:bg-gray-800/90 uppercase tracking-wider text-[10px] border border-gray-200 dark:border-gray-700 z-10">គ្រប់គ្រង</th>}
                            </tr>
                        </thead>
                        <tbody>
                            {filteredTasks.length === 0 ? (
                                <tr>
                                    <td colSpan={isEditor ? 10 : 9} className="p-24 text-center text-gray-400 dark:text-gray-600 italic border border-gray-200 dark:border-gray-700">
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <span className="font-medium">មិនមានសកម្មភាពសម្រាប់បង្ហាញទេ។</span>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                filteredTasks.map((task, idx) => (
                                    <tr key={task.id} className="hover:bg-brand-light/10 dark:hover:bg-brand/5 even:bg-gray-50/50 dark:even:bg-gray-800/20 transition-all group">
                                        <td className="p-4 text-center text-gray-500 dark:text-gray-500 font-mono font-bold text-xs border border-gray-200 dark:border-gray-700">{idx + 1}</td>
                                        <td className="p-4 font-semibold text-gray-900 dark:text-gray-100 leading-relaxed whitespace-pre-wrap border border-gray-200 dark:border-gray-700">{task.activity}</td>
                                        <td className="p-4 border border-gray-200 dark:border-gray-700">
                                            <div className="flex justify-center">
                                                <div className={cn(
                                                    "px-3 py-1 rounded-full text-[10px] font-bold text-white shadow-sm flex items-center justify-center min-w-[80px]",
                                                    task.typeColor === 'red' ? "bg-rose-500" :
                                                    task.typeColor === 'yellow' ? "bg-amber-400 text-gray-900" :
                                                    task.typeColor === 'green' ? "bg-emerald-500" :
                                                    "bg-sky-400"
                                                )}>
                                                    {task.documentType === 'meeting' ? 'ប្រជុំ' : 'ផ្តល់មតិ'}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-4 text-center border border-gray-200 dark:border-gray-700">
                                            {task.fileUrl ? (
                                                <div className="flex justify-center">
                                                    <button 
                                                        onClick={() => {
                                                            const fileId = task.fileUrl?.replace('firestore://', '');
                                                            if (fileId) {
                                                                handleViewFile(fileId);
                                                            }
                                                        }}
                                                        className="p-2 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl hover:bg-blue-100 dark:hover:bg-blue-800/50 transition-all shadow-sm group/btn"
                                                        title="មើលឯកសារ"
                                                    >
                                        ...
                                        <td className="p-4 border border-gray-200 dark:border-gray-700">
                                            <div className="flex justify-center">
                                                <span className={cn(
                                                    "px-3 py-1.2 rounded-xl text-[10px] font-bold ring-1 ring-inset shadow-sm flex items-center gap-1.5",
                                                    task.status === '✅ បញ្ចប់រួចរាល់' ? "bg-emerald-50 text-emerald-700 ring-emerald-600/20 dark:bg-emerald-900/20 dark:text-emerald-400 dark:ring-emerald-500/30" :
                                                    task.status === '🏃 កំពុងដំណើរការ' ? "bg-blue-50 text-blue-700 ring-blue-600/20 dark:bg-blue-900/20 dark:text-blue-400 dark:ring-blue-500/30" :
                                                    task.status === '❌ ផ្អាក' ? "bg-rose-50 text-rose-700 ring-rose-600/20 dark:bg-rose-900/20 dark:text-rose-400 dark:ring-rose-500/30" :
                                                    "bg-gray-100 text-gray-600 ring-gray-400/20 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700/30"
                                                )}>
                                                    {task.status}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="p-4 text-center text-gray-600 dark:text-gray-400 font-bold text-xs tracking-tight border border-gray-200 dark:border-gray-700">{task.startDate}</td>
                                        <td className="p-4 text-center text-gray-600 dark:text-gray-400 font-bold text-xs tracking-tight border border-gray-200 dark:border-gray-700">{task.deadline || <span className="opacity-20">-</span>}</td>
                                        <td className="p-4 text-gray-700 dark:text-gray-300 font-bold text-xs border border-gray-200 dark:border-gray-700">{task.responsiblePerson || <span className="opacity-20">-</span>}</td>
                                        <td className="p-4 text-gray-500 dark:text-gray-500 italic text-[10px] leading-relaxed max-w-[120px] truncate border border-gray-200 dark:border-gray-700" title={task.notes}>{task.notes || <span className="opacity-20">-</span>}</td>
                                        {isEditor && (
                                            <td className="p-4 sticky right-0 bg-white dark:bg-gray-900 group-hover:bg-gray-50/80 dark:group-hover:bg-gray-800/80 transition-all border border-gray-200 dark:border-gray-700 z-10">
                                                <div className="flex items-center justify-center gap-1.5">
                                                    <button 
                                                        onClick={() => setEditingTask(task)}
                                                        className="p-2 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-xl transition-all shadow-sm hover:shadow"
                                                        title="កែសម្រួល"
                                                    >
                                                        <FileText className="w-4 h-4" />
                                                    </button>
                                                    <button 
                                                        ...
                                                        className="p-2 text-rose-600 hover:bg-rose-100 dark:hover:bg-rose-900/50 rounded-xl transition-all shadow-sm hover:shadow"
                                                        title="លុប"
                                                    >
                                                        <Trash2 className="w-4 h-4" />
                                                    </button>
          ...
          {isViewingLogs && isAdmin && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
              ...
              <motion.div 
                ...
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-6xl relative z-10 overflow-hidden flex flex-col max-h-[90vh] border border-gray-100 dark:border-gray-800"
              >
                <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-white dark:bg-gray-900 sticky top-0 z-20">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-brand-light dark:bg-brand/20 rounded-lg flex items-center justify-center">
                      <History className="text-brand w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="text-xl font-bold italic">ប្រវត្តិសកម្មភាពអ្នកប្រើប្រាស់</h3>
                      <p className="text-xs text-gray-500 italic">តាមដានរាល់ការបង្កើត កែប្រែ ឬលុបទិន្នន័យក្នុងប្រព័ន្ធ</p>
                    </div>
                  </div>
                  <button onClick={() => setIsViewingLogs(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-gray-50 dark:bg-gray-800/50">
                          <th className="p-3 text-xs font-bold text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700 whitespace-nowrap">កាលបរិច្ឆេទ</th>
                          <th className="p-3 text-xs font-bold text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700 whitespace-nowrap">អ្នកប្រើប្រាស់</th>
                          <th className="p-3 text-xs font-bold text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700 text-center whitespace-nowrap">សកម្មភាព</th>
                          <th className="p-3 text-xs font-bold text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700 whitespace-nowrap">ផ្នែក</th>
                          <th className="p-3 text-xs font-bold text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700 whitespace-nowrap">ព័ត៌មានលម្អិត</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100 dark:divide-gray-800 text-sm">
                        {logs.length === 0 ? (
                          <tr>
                            <td colSpan={5} className="p-10 text-center text-gray-400 italic">មិនទាន់មានសកម្មភាពត្រូវបានកត់ត្រាទុកនៅឡើយទេ។</td>
                          </tr>
                        ) : (
                          logs.map((log) => (
                            <tr key={log.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                              <td className="p-3 whitespace-nowrap font-mono text-[10px] text-gray-500">
                                {log.timestamp ? format(new Date(log.timestamp), 'yyyy-MM-dd HH:mm:ss') : '-'}
                              </td>
                              <td className="p-3">
                                <div className="flex flex-col">
                                  <span className="font-bold text-gray-900 dark:text-gray-100">{log.userEmail}</span>
                              ...
                              <td className="p-3">
                                <span className="font-medium text-gray-600 dark:text-gray-400">{log.collection}</span>
                              </td>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <datalist id="committee-positions">
                          <option value="រដ្ឋមន្ត្រី" />
                          <option value="រដ្ឋលេខាធិការប្រចាំការ" />
                    ...
                    <div className="space-y-3">
                      {committeeRows.map((row, index) => (
                        <CommitteeRowItem 
                          ...
                          row={row}
                          index={index}
                          isUploading={isUploadingCommitteeFile}
                          onUpdate={(id: string, updates: any) => {
                            setCommitteeRows(prev => prev.map(r => r.id === id ? {...r, ...updates} : r));
                          }}
                          onRemove={(id: string) => {
                          ...
                          onFileUpload={(id: string, file: File | null) => {
                            if (file && file.size > 10 * 1024 * 1024) { // 10MB limit
                              showToast('ឯកសារធំពេក (អតិបរមា 10MB)', 'error');
                              return;
                            }
                            setCommitteeRows(prev => prev.map(r => r.id === id ? {...r, file} : r));
                          }}
                          onViewFile={handleViewFile}
                        />
                      ))}
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300">ប្រភេទបេសកកម្ម</label>
                        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl">
                          <button
                            onClick={() => setBulkUpdateType('mission')}
                            className={cn(
                              "px-4 py-1.5 rounded-lg text-xs font-bold transition-all",
                              bulkUpdateType === 'mission' ? "bg-white dark:bg-gray-700 text-red-600 dark:text-red-400 shadow-sm" : "text-gray-500"
                            )}
                          >
                            បេសកកម្ម
                          </button>
                          <button
                            onClick={() => setBulkUpdateType('leave')}
                            className={cn(
                              "px-4 py-1.5 rounded-lg text-xs font-bold transition-all",
                              bulkUpdateType === 'leave' ? "bg-white dark:bg-gray-700 text-yellow-600 dark:text-yellow-400 shadow-sm" : "text-gray-500"
                            )}
                          >
                            ច្បាប់
                          </button>
                          <button
                            onClick={() => setBulkUpdateType('none')}
                            className={cn(
                              "px-4 py-1.5 rounded-lg text-xs font-bold transition-all",
                              bulkUpdateType === 'none' ? "bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 shadow-sm" : "text-gray-500"
                            )}
                          >
                            មិនប្តូរប្រភេទ
                          </button>
                      ...
                      <div className="quill-editor-container bulk-editor dark:bg-gray-800 dark:text-gray-200 rounded-xl overflow-hidden border dark:border-gray-700">
                        <Suspense fallback={<div className="h-40 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-brand" /></div>}>
                          <ReactQuill 
                            theme="snow"
                            value={bulkNoteText}
        ...
        {isEditor && mainView === 'missions' && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 animate-in fade-in slide-in-from-bottom-8 duration-500">
            <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white/20 dark:border-gray-800 p-1.5 rounded-2xl shadow-2xl flex items-center gap-2 ring-1 ring-black/5">
              <div className="px-3 py-1.5 mr-1 border-r border-gray-200 dark:border-gray-800">
                <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">រៀបចំទិន្នន័យ</span>
              </div>
              <button
                onClick={() => setActiveMissionType('mission')}
                className={cn(
                  "flex items-center gap-2 px-4 py-2 rounded-xl transition-all text-sm font-bold",
                  activeMissionType === 'mission' 
                    ? "bg-red-600 text-white shadow-[0_8px_16px_-6px_rgba(220,38,38,0.4)] scale-105" 
                    : "text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800"
                )}
              >
                <Calendar className={cn("w-4 h-4", activeMissionType === 'mission' ? "animate-pulse" : "")} />
                បេសកកម្ម
              </button>
              <button
                onClick={() => setActiveMissionType('leave')}
                className={cn(
                  "flex items-center gap-2 px-4 py-2 rounded-xl transition-all text-sm font-bold",
                  activeMissionType === 'leave' 
                    ? "bg-yellow-500 text-white shadow-[0_8px_16px_-6px_rgba(234,179,8,0.4)] scale-105" 
                    : "text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800"
                )}
              >
                <AlertCircle className={cn("w-4 h-4", activeMissionType === 'leave' ? "animate-pulse" : "")} />
                ច្បាប់
              </button>
            </div>
          </div>
        ...
        {isEditor && mainView === 'missions' && (
          <div className="fixed bottom-8 right-8 z-40 animate-in fade-in slide-in-from-right-8 duration-500">
            <button 
              onClick={() => setIsAddingOfficial(true)}
              className="bg-brand text-white p-4 rounded-2xl shadow-xl hover:scale-110 active:scale-95 transition-all group"
              title="បន្ថែមមន្ត្រី"
            >
        ...
        {(isAddingTask || editingTask) && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl dark:shadow-none border border-gray-200 dark:border-gray-800 w-full max-w-xl relative z-10 overflow-hidden"
            >
              <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-brand-light dark:bg-brand/10 rounded-xl flex items-center justify-center">
                    <ClipboardCheck className="text-brand w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold dark:text-white">
                      {editingTask ? 'កែសម្រួលសកម្មភាព' : 'បន្ថែមសកម្មភាពថ្មី'}
                    </h3>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5">ព័ត៌មានលម្អិតសកម្មភាព</p>
                  </div>
                </div>
                <button 
                  onClick={() => {
              ...
              <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                  <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">សកម្មភាពការងារ</label>
                      <textarea 
                          value={editingTask ? editingTask.activity : newTask.activity}
                          onChange={e => {
                              if (editingTask) setEditingTask({...editingTask, activity: e.target.value});
                              else setNewTask({...newTask, activity: e.target.value});
                          }}
                          className="w-full px-5 py-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all text-sm min-h-[120px] font-semibold leading-relaxed"
                          placeholder="បញ្ចូលឈ្មោះសកម្មភាព..."
                      />
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">ប្រភេទលិខិត</label>
                          <select 
                              value={editingTask ? (editingTask.documentType || 'meeting') : (newTask.documentType || 'meeting')}
                              onChange={e => {
                                  const val = e.target.value as 'meeting' | 'feedback';
                                  if (editingTask) setEditingTask({...editingTask, documentType: val});
                                  else setNewTask({...newTask, documentType: val});
                              }}
                              className="w-full px-5 py-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all text-sm font-bold appearance-none cursor-pointer"
                          >
                              <option value="meeting">ប្រជុំ</option>
                              <option value="feedback">ផ្តល់មតិ</option>
                          </select>
                      </div>
                      <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">ពណ៌សម្គាល់</label>
                          <div className="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-2 rounded-2xl">
                              {[
                                  ...
                                  <button
                                      key={c.id}
                                      onClick={() => {
                                          if (editingTask) setEditingTask({...editingTask, typeColor: c.id});
                                          else setNewTask({...newTask, typeColor: c.id});
                                      }}
                                      className={cn(
                                          "w-8 h-8 rounded-full transition-all flex items-center justify-center",
                                          c.color,
                                          (editingTask ? editingTask.typeColor : newTask.typeColor) === c.id 
                                              ? "ring-4 ring-offset-2 ring-brand dark:ring-offset-gray-900 scale-110" 
                                              : "hover:scale-105 active:scale-95"
                                      )}
                                  >
                                      {(editingTask ? editingTask.typeColor : newTask.typeColor) === c.id && <Check className="w-4 h-4 text-white" />}
                                  </button>
                              ))}
                          </div>
                      </div>
                  ...
                  <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">ស្ថានភាពបច្ចុប្បន្ន</label>
                      <select 
                          value={editingTask ? editingTask.status : newTask.status}
                          onChange={e => {
                              const val = e.target.value as TaskTracking['status'];
                              if (editingTask) setEditingTask({...editingTask, status: val});
                              else setNewTask({...newTask, status: val});
                          }}
                          className="w-full px-5 py-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all text-sm font-bold appearance-none cursor-pointer"
                      >
                          <option value="⚠️ មិនទាន់ចាប់ផ្តើម">⚠️ មិនទាន់ចាប់ផ្តើម</option>
                  ...
                  <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">ឯកសារភ្ជាប់ (Upload)</label>
                      <div className="relative group">
                          <input 
                              type="file"
                              onChange={async (e) => {
                                  ...
                                  if (file) {
                                      try {
                                          showToast('កំពុងផ្ទុកឡើងឯកសារ...', 'info');
                                          const url = await uploadFileWithFallback(file, 0);
                                          if (editingTask) setEditingTask({...editingTask, fileUrl: url});
                                          else setNewTask({...newTask, fileUrl: url});
                                          showToast('ផ្ទុកឡើងបានជោគជ័យ');
                                      } catch (err) {
                                          showToast('ផ្ទុកឡើងមិនបានជោគជ័យ', 'error');
                                      }
                                  }
                              }}
                              className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                          />
                          <div className="w-full px-5 py-3 border-2 border-dashed border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/30 text-gray-500 rounded-2xl group-hover:border-brand group-hover:bg-brand-light/20 transition-all flex items-center justify-center gap-3">
                              {(editingTask ? editingTask.fileUrl : newTask.fileUrl) ? (
                                  <>
                                      <CheckCircle2 className="w-5 h-5 text-emerald-500" />
                                      <span className="text-sm font-bold text-emerald-600">ឯកសារត្រូវបានភ្ជាប់រួចរាល់</span>
                                  </>
                              ) : (
                                  <>
                                      <Upload className="w-5 h-5" />
                                      <span className="text-sm font-bold">ចុច ឬទាញដាក់ឯកសារនៅទីនេះ</span>
                                  </>
                              )}
                          </div>
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">អ្នកទទួលខុសត្រូវ</label>
                          <input 
                              type="text"
                              value={editingTask ? (editingTask.responsiblePerson || '') : (newTask.responsiblePerson || '')}
                              onChange={e => {
                                  if (editingTask) setEditingTask({...editingTask, responsiblePerson: e.target.value});
                                  else setNewTask({...newTask, responsiblePerson: e.target.value});
                              }}
                              className="w-full px-5 py-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all text-sm font-bold"
                              placeholder="..."
                          />
                      </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">ថ្ងៃចាប់ផ្ដើម</label>
                          <input 
                              type="date"
                              value={editingTask ? editingTask.startDate : newTask.startDate}
                              onChange={e => {
                                  if (editingTask) setEditingTask({...editingTask, startDate: e.target.value});
                                  else setNewTask({...newTask, startDate: e.target.value});
                              }}
                              className="w-full px-5 py-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all text-sm font-bold"
                          />
                      </div>
                      <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">ថ្ងៃផុតកំណត់</label>
                          <input 
                              type="date"
                              value={editingTask ? editingTask.deadline : newTask.deadline}
                              onChange={e => {
                                  if (editingTask) setEditingTask({...editingTask, deadline: e.target.value});
                                  else setNewTask({...newTask, deadline: e.target.value});
                              }}
                              className="w-full px-5 py-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all text-sm font-bold"
                          />
                      </div>
                  ...
                  <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">កំណត់ចំណាំបន្ថែម</label>
                      <input 
                          type="text"
                          value={editingTask ? (editingTask.notes || '') : (newTask.notes || '')}
                          onChange={e => {
                              if (editingTask) setEditingTask({...editingTask, notes: e.target.value});
                              else setNewTask({...newTask, notes: e.target.value});
                          }}
                          className="w-full px-5 py-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all text-sm"
                          placeholder="..."
                      />
                  </div>
              </div>
              ...
              <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 flex gap-4">
                <button 
                  ...
                  className="flex-1 px-6 py-3.5 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 rounded-2xl font-bold hover:bg-white dark:hover:bg-gray-800 transition-all shadow-sm active:scale-95"
                >
                  បោះបង់
                </button>
                <button 
                  onClick={editingTask ? handleUpdateTask : handleAddTask}
                  className="flex-1 px-6 py-3.5 bg-brand text-white rounded-2xl font-bold hover:bg-brand-hover shadow-lg shadow-brand/20 transition-all flex items-center justify-center gap-2 active:scale-95"
                >
                  <Save className="w-5 h-5" />
                  {editingTask ? 'រក្សាទុកការផ្លាស់ប្តូរ' : 'រក្សាទុកសកម្មភាព'}
                </button>
              </div>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              <div className="p-6 text-center">
                ...
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">បញ្ជាក់ការលុប</h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  តើអ្នកពិតជាចង់លុបសកម្មភាពនេះមែនទេ?
                </p>
                <div className="flex gap-3">
                  <button 
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              <div className="p-6 text-center">
                ...
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">បញ្ជាក់ការលុប</h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  តើអ្នកពិតជាចង់លុបព័ត៌មាននេះមែនទេ?
                </p>
                <div className="flex gap-3">
                  <button 
                    onClick={() => setDeletingCommitteeId(null)}
                    className="flex-1 px-4 py-2.5 border border-gray-300 dark:border-gray-700 rounded-xl font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all"
                  >
                    បោះបង់
                  </button>
                  <button 
                    onClick={() => handleDeleteCommittee(deletingCommitteeId)}
                    className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-xl font-medium hover:bg-red-700 transition-all shadow-sm"
                  >
                    លុបចេញ
                  </button>
                                ...
                                {exportConfig.columns.days && (
                                  <td className="p-2 text-center">
                                    <div className="flex flex-wrap gap-0.5 justify-center">
                                      {mission?.days.map(d => (
                                        <span key={`m-${d}`} className="w-4 h-4 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-[2px] flex items-center justify-center text-[8px] font-bold">
                                          {d}
                                        </span>
                                      ))}
                                      {mission?.leaves?.map(d => (
                                        <span key={`l-${d}`} className="w-4 h-4 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 rounded-[2px] flex items-center justify-center text-[8px] font-bold">
                                          {d}
                                        </span>
                                      ))}
                                      {(!mission || (mission.days.length === 0 && (!mission.leaves || mission.leaves.length === 0))) && <span className="text-gray-300 dark:text-gray-600">-</span>}
                                    </div>
                                  </td>
                                )}
                              </tr>
